...
def optimize_coord_encoding(values: np.ndarray, dx: int) -> CoordEncoding:
    ...
    Returns:
        CoordEncoding: A dictionary containing the zarr compressor and filters to use for encoding the coordinates.
...
def create_empty_datacube(
    ...
    var_encoding = {
        name: {"chunks": (chunk_size, chunk_size), "compressor": Blosc(cname="zstd"), **var_encoding[name]}
        for name in data_vars
    }
    encoding = {
